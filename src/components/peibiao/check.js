...
const isPeiBiaoLocationByPlatform = function (platform) {
  return (
    new RegExp(`${platform}\\.rainclassroom\\.com`).test(window.location.host) ||
    new RegExp(`${platform}-example\\.rainclassroom\\.com`).test(window.location.host) ||
    new RegExp(`${platform}\\.yuketang\\.cn`).test(window.location.host) ||
    new RegExp(`xuetangxpeibiaotest=${platform}`).test(window.location.search)
  );
};
  ...
  if (window.xuetangxpeibiao.platform) {
    const addPeiBiaoGlobalLoading = function (data) {
      if (data.platform && document.body && !data.globalLoading) {
        const _PeiBiaoGlobalLoading = Vue.extend({extends: PeiBiaoGlobalLoading,props: {
            platform: {
              type: String,
